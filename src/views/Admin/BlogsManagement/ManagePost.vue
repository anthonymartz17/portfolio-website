			...
			switch (action.text) {
				...
				case "Hide":
					try {
						...
						await this.updatePostVisibility({
							postId: action.id,
		...
		<div class="blog-list-body">
			...
			<template v-else>
				...
				>
					<div
						class="loading"
						v-if="loading.isLoading && loading.postId == blog.id"
					>
						<img src="/icons/loading.gif" alt="loading animation" />
					</div>

					<template v-else>
						<div v-if="!blog.isPublic" class="unpublished">Unpublished</div>
						<div class="gear">
							<MartzIcon icon="gear" size="30" />
						</div>
						<div v-if="blog.thumbnail_data" class="blog-img-container">
							<img
								:src="blog.thumbnail_data.dataURL"
